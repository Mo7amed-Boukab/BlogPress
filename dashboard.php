...
function addArticle($conn, $title, $content, $image, $userId) {
...
function updateArticle($conn, $title, $content, $image, $editId) {
    $stmt = $conn->prepare("UPDATE articles SET title = ?, content = ?, image = ? WHERE id = ?");
    $stmt->bind_param("sssi", $title, $content, $image, $editId);
    return $stmt->execute();
}
...
if(isset($_POST['add'])) {
    ...
    addArticle($conn, $title, $content, $image, $userId);
    $title= $content = $image= '';
}
...
if (isset($_GET['edit_id'])) {
    ...
    if(isset($_POST['update'])) {
        ...
        updateArticle($conn, $title_, $content_, $image_, $editId);
